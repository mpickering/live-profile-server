...
setSessionEndTime i t = do
  ...
  case ms of
    ...
    Just sess -> do
      let endField = DBField (Proxy :: Proxy '("end", Maybe UTCTime)) :: EntityField Session (Maybe UTCTime)
      let mt = sess ^. rlens @'("end", Maybe UTCTime) . rfield
      whenNothing mt $
...
setSessionEndTimeWithError i t er = do
  ...
  case ms of
    ...
    Just sess -> do
      ...
      let merr = sess ^. rlens @'("error", Maybe Text) . rfield
      whenNothing merr $
...
openSession (Entity i conn) = do
  ...
  let host = conn ^. rlens @'("host", Text) . rfield
  let port = conn ^. rlens @'("port", Word) . rfield
...
deleteSession i = do
  ...
  case msess of
    Nothing -> return ()
    Just sess -> do
      let logi = sess ^. rlens @'("log", EventLogId) . rfield
