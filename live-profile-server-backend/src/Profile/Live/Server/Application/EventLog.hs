...
import GHC.RTS.Events.Binary as E
...
parseEventLog bs = fmap fst (E.readEventLog bs)
...
parseEventLogInc bs consumer = do
  ...
  case res of
    ...
    Right parser -> do
        cancel <- runDB $ consumer 1.0 $ ParseHeader undefined
  ...
  parseChunkes = do
    (res, compl) <- foldlM parseChunk (Right E.decodeEventLog, 0) $ B.toChunks bs
  ...
  parseChunk (Right !p, !compl) !chunk = do
    ...
    go !parser = do
      case parser of
        ...
        E.Consume k -> return $ Right (k chunk)
        E.Done _l -> return $ Right parser
        E.Error _l err -> return . Left $ ParseError err
